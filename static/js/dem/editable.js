CORE("dem", {
  ...
  var dem = {
    ...
    _text: function(){
      this.text = CORE.pencil.paper.text(0, 0, "привет!");
      this.text.attr(CORE.dem.options.top_text);
      this._editable(this.text);
      this._text_coord(this.text);

      this.bottom_text = CORE.pencil.paper.text(0, 0, "прикол!");
      this.bottom_text.attr(CORE.dem.options.bottom_text);
      this._text_coord(this.bottom_text, CORE.dem.options.bottom_text_margin);
    },
    ...
    _text_coord: function(text, shift){
      shift = shift || 0;
      var text_shift = parseInt(CORE.dem.options.top_text["font-size"]);
    ...
    _center: function(elem, options){
      ...
      var shift = {
        ...
        y: parseInt(((CORE.pencil.paper.height - CORE.dem.options.text_height) - elem_box.height) / 2),
      };
